...
一个面向演示与练手交付的前后端分离新闻平台项目。

项目包含新闻分类、新闻列表与详情、收藏、浏览历史，以及一个优先查询本地新闻数据的 AI 问答接口。当前仓库已经补齐了本地开发、Docker 演示、测试与演示数据重建的基础链路，适合继续做项目收尾、作品展示和后续扩展。
...
如果你准备把这个项目作为课程作业升级版、练手作品或 GitHub 展示项目，当前最稳的使用方式是：

1. 用 Docker 跑完整演示环境
2. 用本地 `seed_demo_data.py --reset` 保持演示数据稳定
3. 演示前跑一遍 `pytest`
4. 重点展示首页、详情、收藏、历史、AI 问答和 Docker 一键启动能力
